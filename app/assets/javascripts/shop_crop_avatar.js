$(document).on("change", "input#shop_avatar", function(event){
  ...
  reader.onload = function(e){
    ...
    img.onload = function() {
      open_shop_crop_avatar_modal(img.src, img_tag);
    }
  };
  reader.readAsDataURL(file);
});

function open_shop_crop_avatar_modal(img, img_tag){
  $.magnificPopup.open({
    items: {
      type: "inline",
      closeBtnInside: true,
      src: "#crop_avatar_shop"
    },
    callbacks: {
      open: function() {
        $("#crop_avatar_shop").show();
        $("#shop_avatar_crop").attr("src", img);
      ...
      close: function() {
        var img = new Image();
        img.src = $("#shop_avatar_crop").cropper("getCroppedCanvas").toDataURL("image/png");
        img.onload = function() {
          var img_src = crop_image(img);
          img_tag.attr("src", img_src);
        }
        $("#crop_avatar_shop").hide();
      }
    }
  });
}

function crop_shop_avatar(){
  var $crop_x = $("input#shop_crop_avatar_x"),
    $crop_y = $("input#shop_crop_avatar_y"),
    $crop_w = $("input#shop_crop_avatar_w"),
    $crop_h = $("input#shop_crop_avatar_h");
  $("#shop_avatar_crop").cropper({
    viewMode: 1,
    aspectRatio: 1,
    background: false,
    zoomable: false,
